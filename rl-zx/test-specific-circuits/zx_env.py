...
class ZXEnv(gym.Env):
    ...
    def step(self, action):
        ...
        try:
            ...
            circuit_data = self.initial_stats
        ...
        if (
            ...
            circuit = self.basic_optimise_bis(c2)
            ...
            print("Win vs Pyzx: ", win_vs_pyzx, " Episode Gates: ", new_gates_cflow, "Cflow_gates: ", self.pyzx_gates, "Initial Gates", self.initial_stats[self.gate_type], "Episode Len", self.episode_len, "Opt Episode Len", self.opt_episode_len)
            return (
                self.graph,
                reward,
                done,
    ...
    def reset(self, seed=None, options=None):
        ...
        while not valid_circuit:
            ...
            circuit_korbinian =  self.basic_optimise_bis(zx.extract_circuit(graph_korbinian, up_to_perm=True))
            ...
            self.pyzx_data = self.get_data(flow_circuit)# AquÃƒÂ­ cridavem flow_opt al circuit, ara el cridem al graph
    ...
    def policy_obs(self):
        ...
        for node in sorted(p_graph.nodes):
            ...
            if real_node in self.graph.inputs():
                ...
                if oh_phase_idx not in range(0,8) and self.graph.phase(real_node) != 0: #AixÃƒÂ² s'ha de canviar o entendre bÃƒÂ© si el que es vol ÃƒÂ©s descartar fases contÃƒÂ­nues.
                    node_feature[10] = 1.0
                else:
